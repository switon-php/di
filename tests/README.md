...
Test suite for the Switon DI package, organized by test type and speed.
...
vendor/bin/phpunit --configuration tests/phpunit.xml.dist --testsuite=performance
```

`Unit/` and `Integration/` are the normal package suites. `Performance/` is opt-in and should be run explicitly.
...
- Follow naming convention: `Di[Component]Test.php`
